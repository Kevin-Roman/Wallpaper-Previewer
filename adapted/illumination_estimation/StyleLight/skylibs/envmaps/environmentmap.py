...
class EnvironmentMap:
    def __init__(self, im, format_=None, copy=True, color=True):
        ...
        assert format_ is not None, "Please provide format (metadata file not found)."
        assert format_.lower() in SUPPORTED_FORMATS, "Unknown format: {}".format(
            format_
        )
        ...
        if isinstance(im, str):
            ...
            raise Exception(
                "Could not understand input. Please provide a "
        ...
        if self.format_ in ["sphere", "angular", "skysphere", "skyangular"]:
            ...
            assert (
                2 * self.data.shape[0] == self.data.shape[1]
            ), "LatLong format width should be twice the height"
        elif self.format_ == "skylatlong":
            assert (
                4 * self.data.shape[0] == self.data.shape[1]
    ...
    def fromSkybox(cls, top, bottom, left, right, front, back):
        ...
        cube[3 * basedim : 4 * basedim, basedim : 2 * basedim] = np.fliplr(
            np.flipud(back)
        )
        ...
        cube[2 * basedim : 3 * basedim, basedim - 1] = left[-1, ...][
            ::-1
        ]  # bottom-left
    ...
    def solidAngles(self):
        ...
        a = np.vstack(
            (dx[:-1, :-1].ravel(), dy[:-1, :-1].ravel(), dz[:-1, :-1].ravel())
        )
    ...
    def interpolate(self, u, v, valid=None, order=1, filter=True, return_coor=False):
        ...
        source[0, 1:-1] = self.data[0, :]
        source[0, 0] = self.data[0, 0]
        source[0, -1] = self.data[0, -1]
        source[-1, 1:-1] = self.data[-1, :]
        source[-1, 0] = self.data[-1, 0]
        source[-1, -1] = self.data[-1, -1]
        source[1:-1, 0] = self.data[:, 0]
        source[1:-1, -1] = self.data[:, -1]

        # To avoid displacement due to the padding
        u += 1.0 / self.data.shape[1]
        v += 1.0 / self.data.shape[0]
        # flatten, then float (0~1)-->int index
        target = np.vstack(
            (v.flatten() * self.data.shape[0], u.flatten() * self.data.shape[1])
        )
    ...
    def setBackgroundColor(self, color, valid):
        ...
        assert (
            valid.shape[:2] == self.data.shape[:2]
        ), "`valid` must be the same size as the EnvironmentMap."
        ...
        if grayscale:
            self.data[:, :][np.invert(valid)] = self.backgroundColor.dot(
    ...
    def convertTo(self, targetFormat, targetDim=None):
        ...
        """
        assert targetFormat.lower() in SUPPORTED_FORMATS, "Unknown format: {}".format(
            targetFormat
        )
    ...
    def rotate(self, format, input_):
        ...
        """
        assert format.upper() in ROTATION_FORMATS, "Unknown rotation type '{}'".format(
            format
        )
        ...
        dx, dy, dz = (
            ptR[0].reshape(dx.shape),
            ptR[1].reshape(dy.shape),
            ptR[2].reshape(dz.shape),
        )
    ...
    def resize(self, targetSize, order=1):
        ...
        for i in range(2):
            _size.append(
                targetSize[i] / self.data.shape[i]
                if targetSize[i] > 1.0
                else targetSize[i]
            )
    ...
    def getMeanLightVectors(self, normals):
        ...
        meanlight = np.nansum(
            xyz[..., np.newaxis] * meanlight[..., np.newaxis].transpose((0, 1, 3, 2)),
            (0, 1),
        )
    ...
    def embed(self, vfov, rotation_matrix, image):
        ...
        vfov_half_rad = vfov / 2.0 * np.pi / 180.0
    ...
    def project(
        ...
        coords = self._cameraCoordinates(
            vfov, rotation_matrix, ar, resolution, projection, mode
        )
        ...
        if mode == "mask":
            # mask_ = coords
            target_masked = (
                coords.reshape(coords.shape[0], coords.shape[1], 1) * target.data
            )
            return coords
    ...
    def Fov2MaskedPano(
        ...
        coor_ = coor.reshape(-1, resolution[::-1][0], resolution[::-1][1]).astype(
            np.int32
        )
    ...
    def _cameraCoordinates(
        ...
        if projection == "orthographic":
            vfov = np.arctan(np.sin(vfov * np.pi / 180.0)) * 180 / np.pi

        # aspect ratio in pixels
        hfov = 2 * np.arctan(np.tan(vfov * np.pi / 180.0 / 2) * ar) * 180 / np.pi

        # Project angle on the sphere to the +Z plane (distance=1 from the camera)
        mu = np.tan(hfov / 2.0 * np.pi / 180.0)
        mv = np.tan(vfov / 2.0 * np.pi / 180.0)
...
def downscaleEnvmap(nenvmap, sao, sat, times):
    ...
    nenvmap.data = (
        sy[2**times :: 2**times, :, ...] - sy[: -(2**times) : 2**times, :, ...]
    )
