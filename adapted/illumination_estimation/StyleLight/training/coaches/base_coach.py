...
from external.PTI.configs import global_config, hyperparameters, paths_config
from external.PTI.criteria import l2_loss
from external.PTI.criteria.localitly_regulizer import Space_Regulizer
from external.PTI.models.e4e.psp import pSp
from external.PTI.utils.log_utils import log_image_from_w
from external.PTI.utils.models_utils import load_old_G, toogle_grad
    ...
    def calc_loss(
        ...
        if hyperparameters.pt_l2_lambda > 0:
            ...
            if True:  #
                ...
                l2_loss_val = l2_loss.l2_loss(
                    generated_images, real_images
                ) + 10 * l2_loss.l2_loss(
                    real_images * light_mask, generated_images * light_mask
                )
            else:
                l2_loss_val = l2_loss.l2_loss(
                    generated_images, real_images
                )  # +10*l2_loss.l2_loss(real_images*light_mask, generated_images*light_mask)
