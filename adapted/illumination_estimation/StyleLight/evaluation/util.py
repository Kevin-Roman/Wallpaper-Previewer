...
class PanoramaHandler(object):
    ...
    def rgb_to_intenisty(rgbs):
        intensity = (
            0.2126 * rgbs[..., 0] + 0.7152 * rgbs[..., 1] + 0.0722 * rgbs[..., 0]
        )
        return intensity
    ...
    def read_exr(exr_path):
        ...
        rgba = [
            np.fromstring(File.channel(c, PixType), dtype=np.float32) for c in "RGBA"
        ]
    ...
    def read_hdr(hdr_path):
        hdr_img = cv2.imread(
            hdr_path,
            flags=cv2.IMREAD_UNCHANGED | cv2.IMREAD_ANYCOLOR | cv2.IMREAD_ANYDEPTH,
        )
        hdr_img = hdr_img[..., ::-1]
        return hdr_img
    ...
    def prepare_gt_panorama(hdr_img, threshold=None):
        weight = PanoramaHandler.generate_steradian(
            height=hdr_img.shape[0], width=hdr_img.shape[1]
        )
        hdr_intensity = PanoramaHandler.rgb_to_intenisty(hdr_img)

        if threshold is None or threshold < 0.0:
            threshold = hdr_intensity.max() / 20.0

        mask = np.where(hdr_intensity < threshold)

        if mask[0].size != 0:
            ambient = np.sum(
                hdr_img[mask] * weight[mask][:, np.newaxis], axis=0, dtype=np.float32
            ) / np.sum(weight[mask], dtype=np.float32)
        else:
    ...
    def resize_panorama(hdr_img, new_shape):
        if isinstance(new_shape, tuple) and len(new_shape) == 2:
            ...
            hdr_img = cv2.resize(
                hdr_img, (2 * new_shape, new_shape), interpolation=cv2.INTER_AREA
            )
        return hdr_img

    @staticmethod
    def crop_panorama(
        hdr_img, fov_deg, crop_image_h=720, crop_image_aspect_ratio="4:3"
    ):
        # img must be float
...
def convert_to_panorama(dirs, sizes, colors):
    ...
    for i in range(nlights):
        lights = lights + (colors[:, 3 * i + 0 : 3 * i + 3][:, :, None, None]) * (
            torch.exp(
                (
                    torch.matmul(dirs[:, 3 * i + 0 : 3 * i + 3], xyz.view(3, -1)).view(
                        -1, xyz.shape[1], xyz.shape[2]
                    )
                    - 1
                )
                / (sizes[:, i]).view(-1, 1, 1)
...
class SinkhornDistance:
    def __init__(self, eps=1e-9, max_iter=5):
        ...
        M = (
            coordsSqr[:, None]
            + coordsSqr[None, :]
            - 2 * torch.matmul(coords, coords.transpose(1, 0))
        )
        M[M < 0] = 0
